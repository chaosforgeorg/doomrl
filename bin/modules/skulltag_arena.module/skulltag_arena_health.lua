...
Skulltag.Healing = {}
Skulltag.Healing.Init = nil
Skulltag.Healing.AddBeingHP = nil

--locals
local GetHPMax = function(arg_being, arg_allowboost)
  if(arg_being:has_property("rune") and arg_being.rune == RUNE_PROSPERITY) then
    return arg_being.hpmax * 2.5
  elseif(arg_allowboost or arg_being.flags[ BF_MEDPLUS ]) then
...
Skulltag.Healing.AddBeingHP = function(arg_being, arg_amount, arg_allowboost)
...
--init code.
Skulltag.Healing.Init = function()
  ...
  items["smed"].OnUse   = function(self,being)
    ...
    if Skulltag.Healing.AddBeingHP(being, being.hpmax / 4, false) then
  ...
  items["lmed"].OnUse   = function(self,being)
    ...
    if Skulltag.Healing.AddBeingHP(being, being.hpmax, false) then
  ...
  items["shglobe"].OnPickup   = function(self,being)
    ...
    if(Skulltag.Healing.AddBeingHP(being, 10, true) == false) then
      being:msg("Nothing happens.")
    else
  ...
  items["lhglobe"].OnPickup   = function(self,being)
    ...
    if(Skulltag.Healing.AddBeingHP(being, 10, true) == false)  then
      ...
      Skulltag.Healing.AddBeingHP(being, math.max(being.hpmax - 10, 0), false)
      being:msg("You feel like new!")
    end
  end
  items["scglobe"].OnPickup   = function(self,being)
    ...
    --Should always be player
    being.tired = false
    Skulltag.Healing.AddBeingHP(being, math.max(being.hpmax, being.hpmax*2 - being.hp), true)

    Skulltag.AnnouncerPlaySound("soulsphere")
  ...
  items["msglobe"].OnPickup   = function(self,being)
    ...
    Skulltag.AnnouncerPlaySound("megasphere")
